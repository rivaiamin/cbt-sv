...
<aside class="fixed left-0 top-0 h-full w-64 bg-surface-container-low flex flex-col p-4 z-50 border-r border-outline-variant/10">
	<div class="mb-8 px-2">
		...
		<p class="text-xs font-medium text-secondary">Quiz: {$examStore.quizId}</p>
	</div>
	<nav class="flex-1 space-y-1">
		<button type="button" class="w-full flex items-center gap-3 px-4 py-3 bg-surface-container-highest text-primary font-semibold rounded-lg transition-all">
	...
	<div class="mt-auto pt-6 border-t border-outline-variant/20">
		<div class="flex items-center gap-3 px-2 mb-6">
			<div class="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold">
				{$examStore.fullName?.charAt(0) || '?'}
			</div>
			<div>
				<p class="text-sm font-bold text-on-surface">{$examStore.fullName}</p>
				<p class="text-xs text-secondary">{$examStore.studentId}</p>
			</div>
		</div>
		<button 
			type="button"
	...
	<header class="sticky top-0 w-full bg-white px-8 py-4 flex justify-between items-center z-40 border-b border-outline-variant/10">
		<div class="flex items-center gap-4">
			...
			<div class="flex items-center gap-2 bg-surface-container-high px-3 py-1 rounded-full">
				...
				<span class="text-sm font-bold text-primary tabular-nums">Review</span>
			</div>
		</div>
		<div class="flex items-center gap-4">
						...
						{#each Array.from($examStore.flagged) as qId (qId)}
							<div class="p-4 bg-white rounded-lg border-l-4 border-tertiary-container hover:bg-surface-container-low transition-colors cursor-pointer group">
								<div class="flex justify-between items-start mb-1">
									<span class="text-xs font-bold text-primary">Question {qId}</span>
								...
								<p class="text-sm text-on-surface line-clamp-2 leading-snug group-hover:text-primary transition-colors">
									Review this item before final submission.
								</p>
							</div>
						{/each}
						{#if flaggedCount === 0}
				...
				<div class="bg-primary text-white p-6 rounded-xl relative overflow-hidden">
					<div class="relative z-10">
						...
						<div class="flex items-center gap-2 mb-4">
							...
							<span class="text-xs font-semibold">Local answers persisted</span>
						</div>
						<p class="text-sm opacity-80 leading-relaxed">
							Security events: {$examStore.securityEventCount}. Data syncs to the operator when online.
						</p>
					</div>
